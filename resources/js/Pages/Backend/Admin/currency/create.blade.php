				...
				<form method="post" class="validate" autocomplete="off" action="{{ route('currency.store') }}" enctype="multipart/form-data">
					...
					<div class="grid grid-cols-12 gap-x-5">
						...
						<div class="md:col-span-6 col-span-12">
							...
							<x-text-input type="text" name="exchange_rate" value="{{ old('exchange_rate') }}" required />
						</div>

						<div class="md:col-span-6 col-span-12 mt-3">
							<x-input-label value="{{ _lang('Base Currency') }}" />
							<select class="w-full" data-selected="{{ old('base_currency') }}" name="base_currency" required>
						...
						<div class="md:col-span-6 col-span-12 mt-3">
							...
							<select class="w-full" data-selected="{{ old('status') }}" name="status" required>
								<option value="">{{ _lang('Select One') }}</option>
