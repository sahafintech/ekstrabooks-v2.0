...
const SYSTEM_FIELDS = [
  ...
  { value: "purchase_cost", label: "Purchase Cost", required: false },
  { value: "selling_price", label: "Selling Price", required: false },
  { value: "descriptions", label: "Description", required: false },
  { value: "initial_stock", label: "Initial Stock", required: false },
  { value: "stock_management", label: "Stock Management", required: false },
  { value: "allow_for_selling", label: "Allow for Selling", required: false },
  { value: "income_account_name", label: "Income Account Name", required: false },
  { value: "allow_for_purchasing", label: "Allow for Purchasing", required: false },
  { value: "expense_account_name", label: "Expense Account Name", required: false },
  { value: "status", label: "Status", required: false },
  { value: "expiry_date", label: "Expiry Date", required: false },
...
export default function Import() {
  ...
  const handleAutoMap = useCallback(() => {
    ...
    const newMappings = {};
    const usedFields = new Set(); // Track which fields have already been mapped
    ...
    previewData.headers.forEach((header) => {
      ...
      // Try to find a match, prioritizing exact matches
      let bestMatch = null;
      let bestScore = 0;
      ...
      for (const field of SYSTEM_FIELDS) {
        ...
        // Exact match with field value (highest priority)
        ...
        // Header starts with field value (e.g., "name" matches "name" but not "income_account_name")
        else if (normalizedHeader === normalizedFieldValue.split("_")[0] && normalizedFieldValue.split("_").length === 1) {
          score = 80;
        }
        // Field value equals header (for simple fields like "code", "name", "type")
        else if (normalizedFieldValue === normalizedHeader && normalizedHeader.length > 2) {
          score = 75;
        }
        // Header contains the full field label (e.g., "product name" contains "name" - but only for short field names)
        else if (normalizedHeader.includes(normalizedFieldLabel) && normalizedFieldLabel.length > 3) {
          score = 60;
        }
        // Field label contains header and header is reasonably specific (more than 4 chars)
        else if (normalizedFieldLabel.includes(normalizedHeader) && normalizedHeader.length > 4) {
          score = 50;
        }
        // Header contains field value parts (e.g., "selling_price" in header matches "selling_price" field)
  ...
  const handleUploadFile = useCallback(() => {
    ...
    setIsUploading(true);
    router.post(route("products.import.upload"), formData, {
      onSuccess: () => {
        setCurrentStep(1);
      },
      onError: (errors) => {
            ...
            {currentStep === 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Select an Excel or CSV file to import products</CardDescription>
                </CardHeader>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      <ul className="text-sm space-y-1">
                        ...
                        <li>• Include "ID" column to update existing products</li>
                        <li>• Supports product types, units, costs, prices, and stock</li>
                      </ul>
                    </AlertDescription>
                  </Alert>
                          ...
                          {previewData.preview_records.map((record, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell className="text-muted-foreground">
                                {record.data.code || <span className="italic">—</span>}
                              </TableCell>
                              <TableCell className="font-medium">
                                {record.data.name || <span className="text-muted-foreground font-normal italic">No name provided</span>}
                              </TableCell>
                              <TableCell>
                    ...
                    <div className="flex gap-3">
                      ...
                      >
                        {isProcessing ? (
                          ...
                          <>
                            Import {previewData.valid_count} Product{previewData.valid_count !== 1 ? 's' : ''}
                          </>
                        )}
                      </Button>
                    </div>
