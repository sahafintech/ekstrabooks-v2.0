<x-app-layout>
	...
	<div class="main-content">
		<x-page-header title="Products & Services" page="user" subpage="list" />

		<div class="box">
			<div class="box-header flex items-center justify-between">
				<h5>{{ _lang('Products & Services') }}</h5>
				<div>
					<x-primary-button>
						<a href="{{ route('products.create') }}"><i class="ri-add-line mr-1"></i>{{ _lang('Add New Product') }}</a>
					</x-primary-button>
			...
			<div class="box-body">
				<div class="table-bordered rounded-sm ti-custom-table-head overflow-auto">
					<form method="POST" action="{{ route('products.all') }}">
						<div class="p-4">
							@csrf
								...
								<div class="ti-modal-content">
									<div class="modal-body">
										<div class="ti-modal-header">
											<h3 class="text-lg font-bold">{{ _lang('Delete All Selected Products') }}</h3>
										...
										<div class="ti-modal-body">
											<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
												{{ __('Are you sure you want to delete all selected products?') }}
											</h2>
										</div>
						...
						<table id="datatable" class="ti-custom-table ti-custom-table-head whitespace-nowrap">
							...
							<tbody>
								...
								<tr data-id="row_{{ $product->id }}">
									<td>
										<input type="checkbox" name="products[]" id="check_{{ $product->id }}" value="{{ $product->id }}">
									</td>
									<td><img src="{{ asset('/uploads/media/'.$product->image) }}" class="w-14" /></td>
									<td>{{ $product->name }}</td>
									<td>{{ $product->type }}</td>
									<td>
									...
									<td>
										...
										@else
										<span class="text-danger">{{ _lang('Disabled') }}</span>
										@endif
									</td>
									<td class="text-center">
										<div class="hs-dropdown ti-dropdown">
											...
											<div class="hs-dropdown-menu ti-dropdown-menu divide-y divide-gray-200" aria-labelledby="hs-dropdown-with-icons">
												<div class="ti-dropdown-divider">
													<a class="ti-dropdown-item" href="{{ route('products.edit', $product['id']) }}">
														<i class="ri-edit-box-line text-lg"></i>
														Edit
													</a>
													<a class="ti-dropdown-item" href="{{ route('products.show', $product['id']) }}">
														<i class="ri-eye-line text-lg"></i>
														View
													</a>
													<a class="ti-dropdown-item" href="javascript:void(0);" data-hs-overlay="#delete-modal" id="delete" data-id="{{ $product['id'] }}">
														<i class="ri-delete-bin-line text-lg"></i>
														Delete
													</a>
												</div>
		...
		<div id="import-modal" class="hs-overlay hidden ti-modal">
			<div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out lg:!max-w-4xl lg:w-full m-3 lg:!mx-auto">
				<div class="ti-modal-content">
					...
					<div class="modal-body">
						<div class="ti-modal-header">
							<h3 class="text-lg font-bold">{{ _lang('Import Products') }}</h3>
							<button class="ti-modal-close" data-hs-overlay="#import-modal">
								<span class="sr-only">Close</span>
								<i class="ri-close-line text-xl"></i>
							</button>
						</div>
						<form action="{{ route('products.import') }}" method="post" id="import_products" enctype="multipart/form-data">
							@csrf
							<div class="ti-modal-body grid grid-cols-12">
								<div class="col-span-12">
									<div class="flex items-center justify-between">
										<x-input-label value="{{ _lang('Products File') }}" />
									...
									<input type="file" class="w-full dropify" name="products_file" class="w-full" required />
								</div>
...
<script>
	$(document).on('click', '#delete', function() {
		var id = $(this).data('id');
		$('#delete-modal form').attr('action', '/user/products/' + id);
	});
