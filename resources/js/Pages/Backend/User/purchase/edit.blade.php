<x-app-layout>
	...
	<div class="main-content">
		...
		<form method="post" class="validate" autocomplete="off" action="{{ route('bill_invoices.update', $id) }}" enctype="multipart/form-data">
			@csrf
						...
						<div class="box-body">
								...
								<div class="lg:col-span-4 col-span-8">
									<div class="grid grid-cols-12 gap-y-3">
										<div class="col-span-12">
											<x-text-input class="text-xl font-bold" type="text" name="title" value="{{ $purchase->title }}" placeholder="{{ _lang('Bill Title') }}" required />
										</div>

										<div class="col-span-12">
											<x-text-input type="text" name="po_so_number" value="{{ $purchase->po_so_number }}" placeholder="{{ _lang('PO / SO Number') }}" />
										</div>
									</div>
								</div>
							</div>
							...
							<div class="grid grid-cols-12">
								<div class="col-span-12 space-y-3">
									<div class="grid grid-cols-12">
										<x-input-label class="xl:col-span-2 col-span-12" value="{{ _lang('Purchase Date') }}" />
										<div class="xl:col-span-5 col-span-12">
											<x-text-input type="text" class="flatpickr-input" id="date" name="purchase_date" value="{{ \Carbon\Carbon::createFromFormat(get_business_option('date_format', request()->activeBusiness->id),$purchase->purchase_date)->format('d-m-Y') }}" required placeholder="Select Date" />
										</div>
									</div>

									<div class="grid grid-cols-12">
										<x-input-label class="xl:col-span-2 col-span-12" value="{{ _lang('Due Date') }}" />
										<div class="xl:col-span-5 col-span-12">
											<x-text-input type="text" class="flatpickr-input" id="date" name="due_date" value="{{ \Carbon\Carbon::createFromFormat(get_business_option('date_format', request()->activeBusiness->id),$purchase->due_date)->format('d-m-Y') }}" required placeholder="Select Date" />
										</div>
									</div>
								</div>
								...
								<div class="md:col-span-7 col-span-12 mt-5">
									<div class="select-vendor">
										<select class="w-full selectize auto-select" data-selected="{{ $purchase->vendor_id }}" name="vendor_id" required>
							...
							<div class="grid grid-cols-12 my-10 gap-x-2">
								...
								<div class="md:col-span-4 col-span-12">
									...
									<select class="w-full auto-select selectize" id="currency" data-selected="{{ $purchase->currency }}" name="currency" required>
										<option value="">{{ _lang('Select One') }}</option>
										{{ get_currency_list() }}
									</select>
								</div>
								<div class="md:col-span-4 col-span-12 {{ $purchase->currency == request()->activeBusiness->currency ? 'hidden' : '' }}">
									<x-input-label value="{{ _lang('Exchange Rate') }}" />
									<x-text-input id="exchange_rate" step="any" type="number" name="exchange_rate" value="{{ $purchase->exchange_rate }}" readonly />
								</div>
							</div>
								...
								<div class="xl:col-start-6 col-span-12 space-y-3">
									<div class="grid grid-cols-12 gap-x-2" id="before-tax">
										...
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="sub_total" id="sub_total" value="{{ $purchase->sub_total }}" readonly />
									...
									<div class="grid grid-cols-12 gap-x-2">
										<div class="md:col-span-6 col-span-12">
											<div class="custom-control custom-checkbox">
												<input type="checkbox" class="custom-control-input" id="withholding_tax" name="withholding_tax" value="1" {{ $purchase->withholding_tax == 1 ? 'checked' : '' }}>
									...
									<div class="grid grid-cols-12 gap-x-2 old-tax {{ count($purchase->taxes) > 0 ? ''  : 'hidden'}}" id="after-tax">
									...
									<div class="grid grid-cols-12 gap-x-2" id="after-tax">
										...
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="discount_amount" id="discount_amount" value="{{ $purchase->discount }}" readonly />
									...
									<div class="grid grid-cols-12 gap-x-2">
										...
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="grand_total" id="grand_total" value="{{ $purchase->grand_total }}" readonly />
									...
									<div class="{{ $purchase->currency == request()->activeBusiness->currency ? 'hidden' : 'grid' }} grid-cols-12 gap-x-2">
										<x-input-label class="md:col-span-6 col-span-12" value="{{ _lang('Converted Total') }}" />
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="converted_total" id="converted_total" value="{{ $purchase->converted_total }}" readonly />
											<span class="md:text-right converted_total_span">
												{{ $purchase->converted_total }}
											</span>
										</div>
									</div>
								</div>
							...
							<div class="grid grid-cols-12">
								...
								<div class="col-span-12">
									...
									<textarea class="w-full" name="note">{{ $purchase->note }}</textarea>
								</div>

								<div class="col-span-12">
									<x-input-label value="{{ _lang('Footer') }}" />
									<textarea class="w-full" name="footer">{!! xss_clean(get_business_option('invoice_footer', $purchase->footer)) !!}</textarea>
								</div>
								...
								<div class="col-span-12 mt-3">
									...
									<div class="col-span-12 grid grid-cols-12">
										<div class="col-span-3">
											<select class="w-full discount_type auto-select" id="discount_type" name="discount_type" data-selected="{{ $purchase->discount_type }}">
												<option value="0">%</option>
												<option value="1">{{ currency_symbol(request()->activeBusiness->currency) }}</option>
											</select>
										</div>
										<input type="number" step="any" class="col-span-9" name="discount_value" id="discount_value" value="{{ $purchase->discount_value }}">
									</div>
								</div>
