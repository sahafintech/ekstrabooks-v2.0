<x-app-layout>
	...
	<div class="main-content">
		<x-page-header title="Bill Invoices" page="user" subpage="list" />

		<div class="grid grid-cols-12">
			<div class="col-span-12">
				<div class="box">
					<div class="box-header flex items-center justify-between">
						<h5>{{ _lang('Bill Invoices') }}</h5>
						<div>
							<x-primary-button>
								<a href="{{ route('bill_invoices.create') }}"><i class="ri-add-line mr-1"></i>{{ _lang('Add New Bill') }}</a>
							</x-primary-button>
							<x-secondary-button data-hs-overlay="#import-modal">
								<i class="ri-upload-cloud-2-line ml-1"></i>
								Import Bills
							</x-secondary-button>
							<a href="{{ route('bill_invoices.export') }}">
								<x-secondary-button>
					...
					<div class="box-body">
						<form method="POST" action="{{ route('bill_invoices.filter') }}">
							@csrf
						...
						<div class="table-bordered rounded-sm ti-custom-table-head overflow-auto mt-3">
							<form method="POST" action="{{ route('bill_invoices.all') }}">
								<div class="p-4">
									@csrf
										...
										<div class="ti-modal-content">
											<div class="modal-body">
												<div class="ti-modal-header">
													<h3 class="text-lg font-bold">{{ _lang('Delete All Selected Bills') }}</h3>
												...
												<div class="ti-modal-body">
													<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
														{{ __('Are you sure you want to delete all selected bills?') }}
													</h2>
												</div>
								...
								<table id="datatable" class="ti-custom-table ti-custom-table-head whitespace-nowrap">
									<thead>
										<tr>
											...
											<th class="text-center">{{ _lang('Approval Status') }}</th>
									...
									<tbody>
										...
										<tr>
											...
											<td>
												{{ formatAmount($purchase->grand_total - $purchase->paid, currency_symbol(request()->activeBusiness->currency), $purchase->business_id) }}
												@if($purchase->grand_total != $purchase->converted_total)
												({{ formatAmount((($purchase->grand_total - $purchase->paid) * $purchase->exchange_rate), currency_symbol($purchase->currency), $purchase->business_id) }})
												@endif
											</td>
											<td>
												{!! xss_clean(purchase_status($purchase)) !!}
											</td>
											<td>
											...
											<td class="text-center">
												<div class="hs-dropdown ti-dropdown">
													...
													<div class="hs-dropdown-menu ti-dropdown-menu divide-y divide-gray-200" aria-labelledby="hs-dropdown-with-icons">
														<div class="ti-dropdown-divider">
															<a class="ti-dropdown-item" href="{{ route('bill_invoices.edit', $purchase['id']) }}">
																<i class="ri-edit-box-line text-lg"></i>
																Edit
															</a>
															<a class="ti-dropdown-item" href="{{ route('bill_invoices.show', $purchase['id']) }}">
		...
		<x-modal>
			<form method="post">
				...
				<div class="ti-modal-body">
					...
					<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
						{{ __('Are you sure you want to delete the bill?') }}
					</h2>
		...
		<div id="import-modal" class="hs-overlay hidden ti-modal">
			<div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out lg:!max-w-4xl lg:w-full m-3 lg:!mx-auto">
				<div class="ti-modal-content">
					...
					<div class="modal-body">
						<div class="ti-modal-header">
							<h3 class="text-lg font-bold">{{ _lang('Import Bills') }}</h3>
							<button class="ti-modal-close" data-hs-overlay="#import-modal">
								<span class="sr-only">Close</span>
								<i class="ri-close-line text-xl"></i>
							</button>
						</div>
						<form action="{{ route('bill_invoices.import') }}" method="post" id="import_bills" enctype="multipart/form-data">
							@csrf
		...
		<div id="approve-modal" class="hs-overlay hidden ti-modal">
			<div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out lg:!max-w-4xl lg:w-full m-3 lg:!mx-auto">
				<div class="ti-modal-content">
					...
					<div class="modal-body">
						<div class="ti-modal-header">
							<h3 class="text-lg font-bold">{{ _lang('Approve Bill') }}</h3>
						...
						<form method="post">
							...
							<div class="ti-modal-body grid grid-cols-12">
								<div class="col-span-12">
									<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
										{{ __('Are you sure you want to approve the bill?') }}
									</h2>
								</div>
							</div>
							<div class="ti-modal-footer">
...
<script>
	$(document).on('click', '#delete', function() {
		var id = $(this).data('id');
		$('#delete-modal form').attr('action', '/user/bill_invoices/' + id);
	});

	$(document).on('click', '#approve', function() {
		var id = $(this).data('id');
		$('#approve-modal form').attr('action', '/user/approve_bill_invoices/' + id);
	});
