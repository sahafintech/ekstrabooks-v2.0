...
const RestoreConfirmationModal = ({ show, onClose, onConfirm, processing }) => (
  <Modal show={show} onClose={onClose}>
    <form onSubmit={onConfirm}>
      <h2 className="text-lg font-medium">
        Are you sure you want to restore this tax?
      </h2>
      <div className="mt-6 flex justify-end">
        <Button
          type="button"
...
const BulkRestoreConfirmationModal = ({ show, onClose, onConfirm, processing, count }) => (
  <Modal show={show} onClose={onClose}>
    <form onSubmit={onConfirm}>
      <h2 className="text-lg font-medium">
        Are you sure you want to restore {count} selected tax{count !== 1 ? 'es' : ''}?
      </h2>
      <div className="mt-6 flex justify-end">
        <Button
          type="button"
...
export default function TrashList({ taxes = [], meta = {}, filters = {} }) {
  ...
  const [taxToRestore, setTaxToRestore] = useState(null);
  ...
  const toggleSelectTax = (id) => {
    if (selectedTaxes.includes(id)) {
      ...
      if (selectedTaxes.length + 1 === taxes.length) {
  ...
  const handleDelete = (e) => {
    ...
    router.delete(route("taxes.permanent_destroy", taxToDelete), {
      preserveState: true,
      onSuccess: () => {
  ...
  const handleBulkDelete = (e) => {
    ...
    router.post(route("taxes.bulk_permanent_destroy"),
      {
        ids: selectedTaxes
      },
      {
  ...
  const handleRestore = (e) => {
    ...
    router.post(route("taxes.restore", taxToRestore), {
      preserveState: true,
      onSuccess: () => {
  ...
  const handleBulkRestore = (e) => {
    ...
    router.post(route("taxes.bulk_restore"),
      {
        ids: selectedTaxes
      },
      {
                ...
                <div className="flex flex-col md:flex-row gap-2">
                    <div>
                        <div className="text-red-500">
                            Total trashed taxes: {meta.total}
                        </div>
                    </div>
                </div>
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="cursor-pointer" onClick={() => handleSort("rate")}>
                      Tax rate {renderSortIcon("rate")}
                    </TableHead>
                    <TableHead className="cursor-pointer" onClick={() => handleSort("tax_number")}>
                      Tax Number {renderSortIcon("tax_number")}
                    </TableHead>
                    <TableHead className="cursor-pointer" onClick={() => handleSort("account.account_name")}>
                      Account {renderSortIcon("account.account_name")}
                    </TableHead>
                      ...
                      <TableRow key={tax.id}>
                        ...
                        <TableActions
                            actions={[
                            {
                                ...
                                onClick: () => handleRestoreConfirm(tax.id)
                            },
                            {
