<x-app-layout>
	...
	<div class="main-content">
		<x-page-header title="Quotation" page="user" subpage="edit" />
						...
						<div class="box-body">
								...
								<div class="lg:col-span-4 col-span-8">
									<div class="grid grid-cols-12 gap-y-3">
										<div class="col-span-12">
											<x-text-input class="text-xl font-bold" type="text" name="title" value="{{ $quotation->title }}" placeholder="{{ _lang('Quotation Title') }}" required />
										</div>

										<div class="col-span-12">
											<x-text-input type="text" name="po_so_number" value="{{ $quotation->po_so_number }}" placeholder="{{ _lang('Sales Order No') }}" />
										</div>
									</div>
								</div>
							</div>
							...
							<div class="grid grid-cols-12">
								<div class="col-span-12 space-y-3">
									<div class="grid grid-cols-12">
										<x-input-label class="xl:col-span-2 col-span-12" value="{{ _lang('Quotation Date') }}" />
										<div class="xl:col-span-5 col-span-12">
											<x-text-input type="text" class="flatpickr-input" id="date" name="quotation_date" value="{{ \Carbon\Carbon::createFromFormat(get_business_option('date_format', request()->activeBusiness->id),$quotation->quotation_date)->format('d-m-Y') }}" required placeholder="Select Date" />
										</div>
									</div>
									...
									<div class="grid grid-cols-12">
										...
										<div class="xl:col-span-5 col-span-12">
											<x-text-input type="text" class="flatpickr-input" id="date" name="expired_date" value="{{ \Carbon\Carbon::createFromFormat(get_business_option('date_format', request()->activeBusiness->id),$quotation->expired_date)->format('d-m-Y') }}" required placeholder="Select Date" />
										</div>
									</div>
								</div>
								<div class="md:col-span-7 col-span-12 mt-5">
									<div class="select-customer">
										<select class="w-full auto-select selectize" data-selected="{{ $quotation->customer_id }}" name="customer_id" required>
							...
							<div class="grid grid-cols-12 my-10 gap-x-2">
								...
								<div class="md:col-span-4 col-span-12">
									...
									<select class="w-full auto-select selectize" id="currency" data-selected="{{ $quotation->currency }}" name="currency" required>
										<option value="">{{ _lang('Select One') }}</option>
										{{ get_currency_list() }}
									</select>
								</div>
								<div class="md:col-span-4 col-span-12 {{ $quotation->currency == request()->activeBusiness->currency ? 'hidden' : '' }}">
									<x-input-label value="{{ _lang('Exchange Rate') }}" />
									<x-text-input id="exchange_rate" type="number" name="exchange_rate" value="{{ $quotation->exchange_rate }}" />
								</div>
							</div>
								...
								<div class="xl:col-start-6 col-span-12 space-y-3">
									<div class="grid grid-cols-12 gap-x-2" id="before-tax">
										...
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="sub_total" id="sub_total" value="{{ $quotation->sub_total }}" readonly />
											<span class="md:text-right sub_total_span">
												{{ formatAmount($quotation->sub_total) }}
											</span>
										</div>
									</div>

									@foreach($quotation->taxes as $tax)
									<div class="grid grid-cols-12 gap-x-2 old-tax {{ count($quotation->taxes) > 0 ? ''  : 'hidden'}}" id="after-tax">
									...
									<div class="grid grid-cols-12 gap-x-2" id="after-tax">
										...
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="discount_amount" id="discount_amount" value="{{ $quotation->discount }}" readonly />
									...
									<div class="grid grid-cols-12 gap-x-2">
										...
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="grand_total" id="grand_total" value="{{ $quotation->grand_total }}" readonly />
											<span class="md:text-right grand_total_span">
												{{ formatAmount($quotation->grand_total) }}
											</span>
										</div>
									</div>

									<div class="{{ $quotation->currency == request()->activeBusiness->currency ? 'hidden' : 'grid' }} grid-cols-12 gap-x-2">
										<x-input-label class="md:col-span-6 col-span-12" value="{{ _lang('Converted Total') }}" />
										<div class="md:col-span-6 col-span-12">
											<x-text-input type="hidden" class="md:text-right border-none" name="converted_total" id="converted_total" value="{{ $quotation->converted_total }}" readonly placeholder="0.00" />
											<span class="md:text-right converted_total_span">
												{{ formatAmount($quotation->converted_total) }}
											</span>
										</div>
									</div>
								</div>
							...
							<div class="grid grid-cols-12">
								<div class="col-span-12">
									...
									<textarea class="w-full" name="note">{{ $quotation->note }}</textarea>
								</div>

								<div class="col-span-12">
									<x-input-label value="{{ _lang('Footer') }}" />
									<textarea class="w-full" name="footer">{!! xss_clean(get_business_option('quotation_footer', $quotation->footer)) !!}</textarea>
								</div>

								<div class="col-span-12 mt-4">
									<x-primary-button type="submit" class="submit-btn">{{ _lang('Update Quotation') }}</x-primary-button>
								</div>
							</div>
						</div>
								...
								<div class="col-span-12 mt-3">
									...
									<div class="col-span-12 grid grid-cols-12">
										<div class="col-span-3">
											<select class="w-full discount_type" id="discount_type" name="discount_type" value="{{ $quotation->discount_type }}">
												<option value="0">%</option>
												<option value="1">{{ currency_symbol(request()->activeBusiness->currency) }}</option>
											</select>
										</div>
										<input type="number" step="any" class="col-span-9" name="discount_value" id="discount_value" value="{{ $quotation->discount_value }}">
									</div>
								</div>
