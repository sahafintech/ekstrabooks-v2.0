...
export default function Edit({ vendors = [], products = [], currencies = [], taxes = [], purchase_return, taxIds = [] }) {
    ...
    const { data, setData, put, processing, errors, reset } = useForm({
        ...
        discount_type: purchase_return.discount_type || "0",
        discount_value: purchase_return.discount_value || 0,
        type: purchase_return.type || "",
        template: purchase_return.template || "",
    ...
    useEffect(() => {
        if (purchase_return && purchase_return.items && purchase_return.items.length > 0) {
            const formattedItems = purchase_return.items.map(item => ({
            ...
            setPurchaseReturnItems(formattedItems);
        }

        // Set the initial currency and exchange rate
        if (purchase_return && purchase_return.currency) {
        ...
        setData({
            ...data,
            product_id: purchase_return.items.map(item => item.product_id),
            product_name: purchase_return.items.map(item => item.product_name),
            description: purchase_return.items.map(item => item.description),
            quantity: purchase_return.items.map(item => item.quantity),
            unit_cost: purchase_return.items.map(item => item.unit_cost),
        })
    ...
    const submit = (e) => {
        ...
        put(route("purchase_returns.update", purchase_return.id), formData, {
            preserveScroll: true,
            onSuccess: () => {
            ...
            onError: (errors) => {
                console.error("Error updating purchase return:", errors);
    ...
    return (
        <AuthenticatedLayout>
            <SidebarInset>
                <PageHeader page="Purchase Returns" subpage="Edit Purchase Return" url="purchase_returns.index" />
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
                                        <div className="md:col-span-6">
                                            ...
                                            <Textarea
                                                ...
                                                onChange={(e) => updatePurchaseReturnItem(index, "description", e.target.value)}
                                                rows={1}
                                            />
                                        </div>
