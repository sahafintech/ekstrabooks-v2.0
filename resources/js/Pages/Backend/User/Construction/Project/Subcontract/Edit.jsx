...
export default function Edit({ vendors = [], projects = [], costCodes = [], taxes = [], currencies = [], base_currency, accounts = [], projectSubcontract, taxIds, theAttachments }) {
  ...
  const { data, setData, post, processing, errors, reset } = useForm({
    ...
    subcontract_no: projectSubcontract.subcontract_no || "",
    project_id: projectSubcontract.project_id || "",
    start_date: parseDateObject(projectSubcontract.start_date),
    end_date: parseDateObject(projectSubcontract.end_date),
    description: projectSubcontract.description || "",
    currency: projectSubcontract.currency || base_currency,
    exchange_rate: projectSubcontract.exchange_rate || 1,
    converted_total: projectSubcontract.converted_total || 0,
    discount_type: projectSubcontract.discount_type || "0",
    discount_value: projectSubcontract.discount_value || 0,
    attachments: [],
    project_task_id: [],
    cost_code_id: [],
    uom: [],
  ...
  // Initialize contract items from existing subcontract
  useEffect(() => {
    if (projectSubcontract && projectSubcontract.tasks && projectSubcontract.tasks.length > 0) {
      const formattedItems = projectSubcontract.tasks.map(item => ({
    ...
    if (projectSubcontract && projectSubcontract.currency) {
      setData('currency', projectSubcontract.currency);
      setExchangeRate(projectSubcontract.exchange_rate);
    }
    ...
    setData({
      ...data,
      project_task_id: projectSubcontract.tasks.map(item => item.project_task_id),
      cost_code_id: projectSubcontract.tasks.map(item => item.cost_code_id),
      uom: projectSubcontract.tasks.map(item => item.uom),
      quantity: projectSubcontract.tasks.map(item => item.quantity),
      unit_cost: projectSubcontract.tasks.map(item => item.unit_cost),
      account_id: projectSubcontract.tasks.map(item => item.account_id),
    });
  }, [projectSubcontract]);
  ...
  const submit = (e) => {
    ...
    post(route("project_subcontracts.update", projectSubcontract.id), formData, {
      preserveScroll: true,
      onSuccess: () => {
  ...
  return (
    <AuthenticatedLayout>
      <SidebarInset>
        <PageHeader page="Subcontracts" subpage="Edit Subcontract" url="project_subcontracts.index" />
