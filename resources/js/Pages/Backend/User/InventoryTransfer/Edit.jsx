...
export default function Edit({ transfer, businesses = [], products = [] }) {
  ...
  const { data, setData, put, processing, errors, reset } = useForm({
    transfer_date: parseDateObject(transfer.transfer_date),
  ...
  // Initialize transfer items from existing transfer
  useEffect(() => {
    ...
    setData({
      ...data,
      product_id: transfer.items.map(item => item.product_id),
      product_name: transfer.items.map(item => item.product_name),
      notes: transfer.items.map(item => item.notes),
      requested_quantity: transfer.items.map(item => item.requested_quantity),
    });
  }, [transfer]);
  ...
  const submit = (e) => {
    ...
    put(route("inventory_transfers.update", transfer.id), formData, {
      preserveScroll: true,
      onSuccess: () => {
  ...
  return (
    <AuthenticatedLayout>
      <SidebarInset>
        <PageHeader page="Inventory Transfers" subpage="Edit Transfer" url="inventory_transfers.index" />
