<x-app-layout>
	...
	<div class="main-content">
		<x-page-header title="Cash Purchase" page="user" subpage="list" />

		<div class="grid grid-cols-12">
			<div class="col-span-12">
				<div class="box">
					<div class="box-header flex items-center justify-between">
						<h5>{{ _lang('Cash Purchase') }}</h5>
						<div>
							<x-primary-button>
								<a href="{{ route('cash_purchases.create') }}"><i class="ri-add-line mr-1"></i>{{ _lang('Add New Purchase') }}</a>
							</x-primary-button>
							<x-secondary-button data-hs-overlay="#import-modal">
								<i class="ri-upload-cloud-2-line ml-1"></i>
								Import Purchases
							</x-secondary-button>
							<a href="{{ route('cash_purchases.export') }}">
								<x-secondary-button>
					...
					<div class="box-body">
						<form method="POST" action="{{ route('cash_purchases.filter') }}">
							@csrf
							<div class="grid grid-cols-12 gap-x-2">
								<div class="lg:col-span-3 col-span-12 mb-2">
									<select class="w-full selectize auto-select" name="vendor_id" data-selected="{{ $vendor_id ?? '' }}">
										<option value="">{{ _lang('All Suppliers') }}</option>
								...
								<!-- status -->
								<div class="lg:col-span-3 col-span-12 mb-2">
									<select class="w-full selectize auto-select" name="approval_status" data-selected="{{ $approval_status ?? '' }}">
						...
						<div class="table-bordered rounded-sm ti-custom-table-head overflow-auto mt-3">
							<form method="POST" action="{{ route('cash_purchases.all') }}">
								<div class="p-4">
									@csrf
									<div class="hs-dropdown ti-dropdown">
										...
										<div class="hs-dropdown-menu ti-dropdown-menu divide-y divide-gray-200" aria-labelledby="hs-dropdown-with-icons">
											...
											<button class="ti-dropdown-item w-full" type="button" data-hs-overlay="#approve-all-modal">
										...
										<div class="ti-modal-content">
											<div class="modal-body">
												<div class="ti-modal-header">
													<h3 class="text-lg font-bold">{{ _lang('Delete All Selected Cash Purchases') }}</h3>
												...
												<div class="ti-modal-body">
													<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
														{{ __('Are you sure you want to delete all selected cash purchases?') }}
													</h2>
												</div>
								...
								<table id="datatable" class="ti-custom-table ti-custom-table-head whitespace-nowrap">
									...
									<tbody>
										...
										<tr>
											<td>
												<input type="checkbox" name="bills[]" id="check_{{ $purchase->id }}" value="{{ $purchase->id }}">
											</td>
											...
											<td>
												...
												<span class="text-warning">{{ _lang('Pending') }}</span>
												@elseif($purchase->approval_status == 1)
												<span class="text-success">{{ _lang('Approved') }}</span>
												@endif
											</td>
											<td>
												<div class="hs-dropdown ti-dropdown">
													...
													<div class="hs-dropdown-menu ti-dropdown-menu divide-y divide-gray-200" aria-labelledby="hs-dropdown-with-icons">
														<div class="ti-dropdown-divider">
															<a class="ti-dropdown-item" href="{{ route('cash_purchases.edit', $purchase['id']) }}">
																<i class="ri-edit-box-line text-lg"></i>
																Edit
															</a>
															<a class="ti-dropdown-item" href="{{ route('cash_purchases.show', $purchase['id']) }}">
																<i class="ri-eye-line text-lg"></i>
																View
															</a>
															<a class="ti-dropdown-item" href="{{ route('cash_purchases.voucher', $purchase['id']) }}">
															...
															<!-- approve -->
															@if($purchase->approval_status == 0)
															<a class="ti-dropdown-item" href="javascript:void(0);" data-hs-overlay="#approve-modal" data-id="{{ $purchase['id'] }}" id="approve">
																<i class="ri-check-line text-lg"></i>
																Approve
															</a>
															@endif
		...
		<div id="import-modal" class="hs-overlay hidden ti-modal">
			<div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out lg:!max-w-4xl lg:w-full m-3 lg:!mx-auto">
				<div class="ti-modal-content">
					...
					<div class="modal-body">
						<div class="ti-modal-header">
							<h3 class="text-lg font-bold">{{ _lang('Import Cash Purchases') }}</h3>
							<button class="ti-modal-close" data-hs-overlay="#import-modal">
								<span class="sr-only">Close</span>
								<i class="ri-close-line text-xl"></i>
							</button>
						</div>
						<form action="{{ route('cash_purchases.import') }}" method="post" id="import_bills" enctype="multipart/form-data">
							@csrf
		...
		<div id="approve-modal" class="hs-overlay hidden ti-modal">
			<div class="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out lg:!max-w-4xl lg:w-full m-3 lg:!mx-auto">
				<div class="ti-modal-content">
					<div class="ti-modal-body hidden" id="modal_spinner">
						<div class="text-center spinner">
					...
					<div class="modal-body">
						<div class="ti-modal-header">
							<h3 class="text-lg font-bold">{{ _lang('Approve Purchase') }}</h3>
							<button class="ti-modal-close" data-hs-overlay="#approve-modal">
								<span class="sr-only">Close</span>
								<i class="ri-close-line text-xl"></i>
							</button>
						</div>
						<form method="post">
							...
							<div class="ti-modal-body grid grid-cols-12">
								<div class="col-span-12">
									<h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
										{{ __('Are you sure you want to approve the purchase?') }}
									</h2>
								</div>
							</div>
							<div class="ti-modal-footer">
								<x-secondary-button type="button" data-hs-overlay="#approve-modal">
									Close
								</x-secondary-button>
								<x-primary-button type="submit" class="submit-btn">
									Approve
...
<script>
	$(document).on('click', '#delete', function() {
		var id = $(this).data('id');
		$('#delete-modal form').attr('action', '/user/cash_purchases/' + id);
	});

	$(document).on('click', '#approve', function() {
		var id = $(this).data('id');
		$('#approve-modal form').attr('action', '/user/approve_cash_purchases/' + id);
	});
