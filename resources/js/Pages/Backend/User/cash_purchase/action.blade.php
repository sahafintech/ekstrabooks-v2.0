<div class="box">
    <div class="box-header flex items-center justify-between">
        <h5>{{ _lang('Bill Invoice') }} #{{ $bill->bill_no }} </h5>
        <div>
            <div class="hs-dropdown ti-dropdown">
                ...
                <div class="hs-dropdown-menu ti-dropdown-menu divide-y divide-gray-200" aria-labelledby="hs-dropdown-with-icons">
                    <div class="ti-dropdown-divider">
                        ...
                        <hr>
                        @if($bill->status != 2)
                        <a href="{{ route('bill_invoices.edit', $bill->id) }}" class="ti-dropdown-item"><i class="far fa-edit mr-2"></i>{{ _lang('Edit') }}</a>
                        @endif
                        <a class="ti-dropdown-item" href="javascript:void(0);" data-hs-overlay="#delete-modal" data-id="{{ $bill['id'] }}" id="delete"><i class="far fa-trash-alt mr-2"></i>{{ _lang('Delete') }}</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="box-body">
        <div class="flex justify-between border border-secondary rounded-md p-3">
            <div>
                ...
                <span>
                    @if($bill->status == 0 && $bill->due_date < date('Y-m-d'))
                        <span class="text-red">{{ _lang('Overdue') }}</span>
                @elseif($bill->status == 0 && $bill->due_date >= date('Y-m-d'))
            ...
            <div>
                ...
                <span>{{ formatAmount($bill->grand_total, currency_symbol($bill->business->currency), $bill->business_id) }}</span>
            </div>
            <div>
                <span>{{ _lang('Amount Due') }}:</span>
                <span>{{ formatAmount($bill->grand_total - $bill->paid, currency_symbol($bill->business->currency), $bill->business_id) }}</span>
            </div>
        </div>
    </div>
</div>
