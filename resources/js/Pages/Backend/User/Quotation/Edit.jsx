...
const getInitialQuotationItems = (quotation) =>
  quotation?.items?.length
    ? quotation.items.map((item) => {
        ...
        return Number(quotation?.is_deffered) === 1 && quotation?.invoice_category === "medical"
          ? {
              ...formattedItem,
              unit_cost: calculateMedicalCoverageRate(formattedItem.coverage_configuration),
            }
          : formattedItem;
      })
...
const buildInitialFormData = (quotation, taxIds) => {
  ...
  return {
    ...
    coverage_configuration: initialQuotationItems.map((item) => item.coverage_configuration),
    taxes: taxIds ?? [],
    _method: "PUT",
  };
};
...
export default function Edit({
  ...
  const [openCoverageConfigurationIndex, setOpenCoverageConfigurationIndex] = useState(
    Number(quotation?.is_deffered) === 1 && quotation?.invoice_category === "medical" && initialQuotationItems.length > 0
      ? 0
      : null
  );
              ...
              <div className="space-x-2">
                <Button
                  ...
                  onClick={() => {
                    ...
                    setOpenCoverageConfigurationIndex(
                      Number(quotation?.is_deffered) === 1 &&
                        quotation?.invoice_category === "medical" &&
                        initialQuotationItems.length > 0
                        ? 0
                        : null
                    );
                  }}
