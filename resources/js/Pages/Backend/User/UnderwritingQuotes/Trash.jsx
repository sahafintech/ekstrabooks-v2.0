...
const DeleteModal = ({ show, onClose, onConfirm, processing }) => (
    <Modal show={show} onClose={onClose}>
        <form onSubmit={onConfirm}>
            <h2 className="text-lg font-medium">Are you sure you want to permanently delete this underwriting quote?</h2>
            <div className="mt-6 flex justify-end">
                <Button type="button" variant="secondary" onClick={onClose} className="mr-3">Cancel</Button>
                <Button type="submit" variant="destructive" disabled={processing}>Permanently Delete</Button>
            </div>
        </form>
    </Modal>
);
...
const DeleteAllModal = ({ show, onClose, onConfirm, processing, count }) => (
    <Modal show={show} onClose={onClose}>
        <form onSubmit={onConfirm}>
            <h2 className="text-lg font-medium">Are you sure you want to permanently delete {count} selected quote{count !== 1 ? "s" : ""}?</h2>
            <div className="mt-6 flex justify-end">
                <Button type="button" variant="secondary" onClick={onClose} className="mr-3">Cancel</Button>
...
const RestoreModal = ({ show, onClose, onConfirm, processing }) => (
    <Modal show={show} onClose={onClose}>
        <form onSubmit={onConfirm}>
            <h2 className="text-lg font-medium">Are you sure you want to restore this underwriting quote?</h2>
            <div className="mt-6 flex justify-end">
                <Button type="button" variant="secondary" onClick={onClose} className="mr-3">Cancel</Button>
                <Button type="submit" variant="default" disabled={processing}>Restore Quote</Button>
            </div>
        </form>
    </Modal>
);
...
const RestoreAllModal = ({ show, onClose, onConfirm, processing, count }) => (
    <Modal show={show} onClose={onClose}>
        <form onSubmit={onConfirm}>
            <h2 className="text-lg font-medium">Are you sure you want to restore {count} selected quote{count !== 1 ? "s" : ""}?</h2>
            <div className="mt-6 flex justify-end">
                <Button type="button" variant="secondary" onClick={onClose} className="mr-3">Cancel</Button>
                <Button type="submit" variant="default" disabled={processing}>Restore Selected</Button>
            </div>
        </form>
    </Modal>
);

const StatusBadge = ({ expired_date }) => {
    const parsedDate = parseDateObject(expired_date);
    const expirationDate = parsedDate ? new Date(parsedDate) : null;
    if (expirationDate) expirationDate.setHours(23, 59, 59, 999);
    const isActive = expirationDate ? expirationDate >= new Date() : false;
    return (
        <span className={isActive ? "text-blue-600" : "text-red-600"}>
...
export default function TrashList({ quotations = [], meta = {}, filters = {}, customers = [] }) {
    ...
    const [quoteToRestore, setQuoteToRestore] = useState(null);
    ...
    const handleSort = (column) => {
        ...
        router.get(route("underwriting_quotes.trash"), { ...filters, sorting: { column, direction } }, { preserveState: true });
    };
    ...
    const handleSearch = (e) => {
        ...
        setSearch(value);
        router.get(route("underwriting_quotes.trash"), buildParams({ search: value }), { preserveState: true });
    };

    const handlePerPageChange = (value) => {
        setPerPage(value);
        router.get(route("underwriting_quotes.trash"), buildParams({ per_page: value }), { preserveState: true });
    };

    const handlePageChange = (page) => {
        setCurrentPage(page);
        router.get(route("underwriting_quotes.trash"), buildParams({ page }), { preserveState: true });
    };

    const handleCustomerChange = (value) => {
        setSelectedCustomer(value);
        router.get(route("underwriting_quotes.trash"), buildParams({ customer_id: value }), { preserveState: true });
    };

    const handleDateRangeChange = (dates) => {
        setDateRange(dates);
        router.get(route("underwriting_quotes.trash"), buildParams({ date_range: dates }), { preserveState: true });
    };

    const handleStatusChange = (value) => {
        setSelectedStatus(value);
        router.get(route("underwriting_quotes.trash"), buildParams({ status: value }), { preserveState: true });
    };
    ...
    const handleDelete = (e) => {
        e.preventDefault();
        setProcessing(true);
        router.delete(route("underwriting_quotes.permanent_destroy", quoteToDelete), {
    ...
    const handleDeleteAll = (e) => {
        e.preventDefault();
        setProcessing(true);
        router.post(route("underwriting_quotes.bulk_permanent_destroy"), { ids: selectedQuotations }, {
            onSuccess: () => {
    ...
    const handleRestore = (e) => {
        e.preventDefault();
        setProcessing(true);
        router.post(route("underwriting_quotes.restore", quoteToRestore), {}, {
            onSuccess: () => { setShowRestoreModal(false); setQuoteToRestore(null); setProcessing(false); },
            onError: () => setProcessing(false),
        });
    };
    ...
    const handleRestoreAll = (e) => {
        e.preventDefault();
        setProcessing(true);
        router.post(route("underwriting_quotes.bulk_restore"), { ids: selectedQuotations }, {
            onSuccess: () => {
        ...
        <AuthenticatedLayout>
            ...
            <SidebarInset>
                <div className="main-content">
                    <PageHeader page="Underwriting Quotes" subpage="Trash" url="underwriting_quotes.index" />
                    <div className="p-4">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                            <div className="text-red-500">Total trashed quotes: {meta.total}</div>
                        ...
                        <div className="mb-4 flex flex-col md:flex-row gap-4 justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <Select value={perPage.toString()} onValueChange={handlePerPageChange}>
                                    <SelectTrigger className="w-[80px]"><SelectValue placeholder="50" /></SelectTrigger>
                                    <SelectContent>
                                    ...
                                    {quotations.length > 0 ? (
                                        quotations.map((quotation) => (
                                            <TableRow key={quotation.id}>
                                                <TableCell>
                                                    <Checkbox checked={selectedQuotations.includes(quotation.id)} onCheckedChange={() => toggleSelect(quotation.id)} />
                                                </TableCell>
                                                ...
                                                <TableCell><StatusBadge expired_date={quotation.expired_date} /></TableCell>
                                                <TableCell className="text-right">
                                                    <TableActions
                                                        actions={[
                                                            { label: "Restore", icon: <RotateCcw className="h-4 w-4" />, onClick: () => { setQuoteToRestore(quotation.id); setShowRestoreModal(true); } },
                                                            { label: "Permanently Delete", icon: <Trash className="h-4 w-4" />, onClick: () => { setQuoteToDelete(quotation.id); setShowDeleteModal(true); }, destructive: true },
                                                        ]}
                                                    />
                                                </TableCell>
                                            </TableRow>
                                        ...
                                        <TableRow>
                                            <TableCell colSpan={8} className="h-24 text-center">No trashed underwriting quotes found.</TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                            </Table>
            ...
            <RestoreAllModal show={showRestoreAllModal} onClose={() => setShowRestoreAllModal(false)} onConfirm={handleRestoreAll} processing={processing} count={selectedQuotations.length} />
        </AuthenticatedLayout>
    );
}
