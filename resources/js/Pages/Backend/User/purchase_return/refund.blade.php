<form method="post" class="ajax-screen-submit" autocomplete="off" action="{{ route('purchase_returns.refund.store', $purchaseReturn->id) }}" enctype="multipart/form-data">
    {{ csrf_field() }}
    ...
    @if($purchaseReturn->grand_total - $purchaseReturn->paid > 0)
    <div class="ti-modal-body grid grid-cols-12 gap-x-5">
        <h1 class="col-span-12 text-xl font-bold">
            Balance: {{ formatAmount($purchaseReturn->grand_total - $purchaseReturn->paid, currency_symbol(request()->activeBusiness->currency), $purchaseReturn->business_id) }}
        </h1>
