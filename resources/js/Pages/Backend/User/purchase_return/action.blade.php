<div class="box">
    <div class="box-header flex items-center justify-between">
        <span class="panel-title">{{ _lang('Purchase Return') }} #{{ $purchase_return->return_number }} </span>
        <div>
            <div class="hs-dropdown ti-dropdown">
                ...
                <div class="hs-dropdown-menu ti-dropdown-menu divide-y divide-gray-200" aria-labelledby="hs-dropdown-with-icons">
                    <div class="ti-dropdown-divider">
                        ...
                        <a href="{{ route('purchase_returns.export_pdf', $purchase_return->id) }}" class="ti-dropdown-item">
                            <i class="ri-file-pdf-line text-lg"></i>
                            {{ _lang('Export PDF') }}
                        </a>
                        <hr>
                        @if($purchase_return->status == 0)
                        <a href="{{ route('purchase_returns.edit', $purchase_return->id) }}" class="ti-dropdown-item">
                            <i class="ri-file-edit-line text-lg"></i>
                            {{ _lang('Edit') }}
                        </a>
                        @endif
                        <a class="w-full ti-dropdown-item" href="javascript:void(0);" data-hs-overlay="#delete-modal" data-id="{{ $purchase_return['id'] }}" id="delete">
    ...
    <div class="box-body">
        <div class="md:flex justify-between border border-secondary rounded-md p-3 mb-3">
            <div>
                <span>{{ _lang('Supplier') }}:</span>
                <span>{{ $purchase_return->vendor->name }}</span>
            </div>
            <div>
                <span>{{ _lang('Grand Total') }}:</span>
                <span>{{ formatAmount($purchase_return->grand_total, currency_symbol($purchase_return->business->currency), $purchase_return->business_id) }}</span>
            </div>
        </div>

        @if($purchase_return->transactions->count() > 0)
        <!-- <table class="table border mt-4">
            ...
            <tbody>
                @foreach($purchase_return->transactions as $transaction)
                <tr>
                    <td>{{ $transaction->trans_date }}</td>
                    <td>{{ $transaction->method }}</td>
                    <td class="text-right">{{ formatAmount($transaction->amount, currency_symbol($transaction->account->currency)) }}</td>
                    <td class="text-right">{{ formatAmount($transaction->ref_amount, currency_symbol($purchase_return->business->currency)) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table> -->
