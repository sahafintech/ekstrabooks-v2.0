<x-app-layout>
	...
	<div class="main-content">
		<x-page-header title="Credit Invoices" page="user" subpage="edit" />
						...
						<div class="box-body">
								...
								<div class="lg:col-span-4 col-span-8">
									<div class="grid grid-cols-12 gap-y-3">
										<div class="col-span-12">
											<x-text-input class="text-xl font-bold" type="text" name="title" value="{{ $invoice->title ?? 'Invoice' }}" placeholder="{{ _lang('Invoice Title') }}" required />
										</div>

										<div class="col-span-12">
											<x-text-input type="text" name="order_number" value="{{ $invoice->order_number }}" placeholder="{{ _lang('Sales Order No') }}" />
										</div>
									</div>
								</div>
							</div>
							...
							<div class="grid grid-cols-12">
								...
								<div class="md:col-span-7 col-span-12 mt-5">
									<div class="select-customer">
										<select class="w-full auto-select selectize" data-selected="{{ $invoice->customer_id }}" name="customer_id" required>
							...
							<div class="grid grid-cols-12 my-10 gap-x-2">
								...
								<div class="md:col-span-4 col-span-12">
									...
									<select class="w-full auto-select selectize" id="currency" data-selected="{{ $invoice->currency }}" name="currency" required>
										<option value="">{{ _lang('Select One') }}</option>
										{{ get_currency_list() }}
									</select>
								</div>
								<div class="md:col-span-4 col-span-12 {{ $invoice->currency == request()->activeBusiness->currency ? 'hidden' : '' }}">
									<x-input-label value="{{ _lang('Exchange Rate') }}" />
									<x-text-input id="exchange_rate" step="any" type="number" name="exchange_rate" value="{{ $invoice->exchange_rate }}" readonly />
								</div>
							</div>
							...
							<div class="grid grid-cols-12">
								...
								<div class="col-span-12 mt-4">
									<x-primary-button type="submit" class="submit-btn">{{ _lang('Update Credit Invoice') }}</x-primary-button>
								</div>
							</div>
						</div>
								...
								<div class="col-span-12 mt-3">
									...
									<div class="col-span-12 grid grid-cols-12">
										<div class="col-span-3">
											<select class="w-full discount_type auto-select" id="discount_type" name="discount_type" data-selected="{{ $invoice->discount_type }}">
												<option value="0">%</option>
												<option value="1">{{ currency_symbol(request()->activeBusiness->currency) }}</option>
											</select>
										</div>
										<input type="number" step="any" class="col-span-9" name="discount_value" id="discount_value" min="0" value="{{ $invoice->discount_value }}">
									</div>
								</div>
