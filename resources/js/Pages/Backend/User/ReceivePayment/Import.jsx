...
export default function Import() {
    ...
    previewData.headers.forEach((header) => {
      ...
      for (const field of SYSTEM_FIELDS) {
        ...
        if (normalizedHeader === normalizedFieldValue) {
          ...
          score = 90;
        } else if ((normalizedHeader.includes("customer") || normalizedHeader.includes("client")) && field.value === "customer_name") {
          score = 85;
        } else if (normalizedHeader.includes("invoice") && field.value === "invoice_number") {
          score = 85;
  ...
  const handleUploadFile = useCallback(() => {
    ...
    setIsUploading(true);
    router.post(route("receive_payments.import.upload"), formData, {
      onSuccess: () => {
        setCurrentStep(1);
      },
      onFinish: () => {
  ...
  const handleGeneratePreview = useCallback(() => {
    ...
    router.post(
      route("receive_payments.import.preview"),
  ...
  const handleConfirmImport = useCallback(() => {
    setIsProcessing(true);
    router.post(
      route("receive_payments.import.execute"),
      { mappings: fieldMappings },
      {
        onSuccess: () => {
          router.visit(route("receive_payments.index"));
        },
        onFinish: () => {
          setIsProcessing(false);
        }
  ...
  const isCustomerMapped = Object.values(fieldMappings).includes("customer_name");
  const isInvoiceMapped = Object.values(fieldMappings).includes("invoice_number");
  ...
  const isAllRequiredMapped = isCustomerMapped && isInvoiceMapped && isAmountMapped && isPaymentDateMapped && isPaymentAccountMapped;

  const steps = ["Upload File", "Map Fields", "Preview & Import"];

  return (
    <AuthenticatedLayout>
      <Head title="Import Receive Payments" />
      <SidebarInset>
        <div className="main-content">
          <PageHeader page="Receive Payments" subpage="Import" url="receive_payments.index" />
            ...
            {currentStep === 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Select an Excel or CSV file to import receive payments</CardDescription>
                </CardHeader>
                <CardContent className="space-y-5">
                  ...
                  <Alert className="border-blue-500/30 bg-blue-500/5">
                    ...
                    <AlertDescription>
                      ...
                      <ul className="text-sm space-y-1 text-blue-600">
                        <li>- A new receive payment record is created for each customer</li>
                        <li>- A new record is also created when payment date, payment account, payment method, or reference changes</li>
                        <li>- Rows with the same customer, date, account, method, and reference become one receive payment</li>
                      </ul>
                    </AlertDescription>
                  </Alert>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      <ul className="text-sm space-y-1">
                        <li>- Required fields: Customer Name, Invoice Number, Amount, Payment Date, Payment Account</li>
                        <li>- Invoice Number must belong to the selected customer</li>
                        <li>- Amount must be greater than 0 and less than or equal to invoice due amount</li>
                  ...
                  <div className="flex items-center justify-between p-4 border rounded-lg">
                    ...
                    <a href="/uploads/media/default/sample_receive_payments.xlsx" download>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Map your file columns to receive payment fields</CardDescription>
                  </div>
                    ...
                    {isAllRequiredMapped ? (
                      ...
                      <>
                        ...
                        <AlertDescription className="text-yellow-600 font-medium">
                          ...
                          {!isCustomerMapped && <span className="ml-1">"Customer Name"</span>}
                          {!isInvoiceMapped && <span className="ml-1">"Invoice Number"</span>}
                        ...
                        <TableBody>
                          {previewData.preview_records.map((record, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell>{record.data.customer_name || "N/A"}</TableCell>
                              <TableCell>{record.data.invoice_number || "N/A"}</TableCell>
                              <TableCell>{record.data.due_amount ?? "N/A"}</TableCell>
