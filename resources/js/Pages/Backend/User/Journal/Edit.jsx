...
export default function Edit({ 
  ...
  const { auth, errors: validationErrors } = usePage().props;
  const activeBusinessCurrency = auth.active_business?.currency || "USD";

  // Prepare initial form values from journal and transactions
  const prepareInitialValues = () => {
    // Map transactions to journal entries format
    const journal_entries = transactions.map((transaction) => ({
      id: transaction.id,
      date: transaction.trans_date,
      account_id: transaction.account_id.toString(),
      debit: transaction.dr_cr === 'dr' ? transaction.transaction_amount.toString() : "",
      credit: transaction.dr_cr === 'cr' ? transaction.transaction_amount.toString() : "",
      description: transaction.description || "",
      customer_id: transaction.customer_id ? transaction.customer_id.toString() : "",
      vendor_id: transaction.vendor_id ? transaction.vendor_id.toString() : "",
    }));

    return {
      date: journal.date || format(new Date(), "yyyy-MM-dd"),
      journal_number: journal.journal_number || "",
      trans_currency: journal.transaction_currency || activeBusinessCurrency,
      journal_entries: journal_entries.length > 0 ? journal_entries : [
        {
  ...
  const { data, setData, post, processing, errors } = useForm(prepareInitialValues());
  ...
  // Handle field change for journal entries
  ...
  // Format currency
  const formatCurrency = (amount, currency = data.trans_currency) => {
  ...
  const isFormBalanced = Math.abs(totals.difference) < 0.001;
                              ...
                              <Popover>
                                ...
                                <PopoverContent className="w-auto p-0" align="start">
                                  <Calendar
                                    ...
                                    onSelect={(date) => handleFieldChange(index, "date", format(date, "yyyy-MM-dd"))}
                                    initialFocus
                                  />
                                </PopoverContent>
                              </Popover>
