...
const SYSTEM_FIELDS = [
  { value: "receipt_number", label: "Invoice Number", required: false, description: "Rows with the same invoice number become one cash invoice with multiple items" },
  { value: "receipt_date", label: "Invoice Date", required: true },
  { value: "customer_name", label: "Customer Name", required: false },
  ...
  { value: "payment_method", label: "Payment Method", required: true },
  { value: "payment_reference", label: "Payment Reference", required: false },
...
export default function Import() {
    ...
    previewData.headers.forEach((header) => {
      ...
      for (const field of SYSTEM_FIELDS) {
        ...
        if (normalizedHeader === normalizedFieldValue) {
          ...
          score = 90;
        } else if ((normalizedHeader.includes("invoice_number") || normalizedHeader.includes("receipt_number")) && field.value === "receipt_number") {
          score = 95;
        } else if (normalizedHeader.includes("invoice") && normalizedHeader.includes("date") && field.value === "receipt_date") {
          score = 95;
        } else if (normalizedHeader.includes("receipt") && normalizedHeader.includes("date") && field.value === "receipt_date") {
          score = 95;
        } else if (normalizedHeader.includes("customer") && field.value === "customer_name") {
          score = 90;
        } else if (normalizedHeader.includes("project") && field.value === "project_name") {
          score = 85;
          ...
          score = 80;
        } else if (normalizedHeader.includes("unit_cost") && field.value === "unit_cost") {
          score = 90;
        } else if (normalizedHeader.includes("price") && field.value === "unit_cost") {
          score = 75;
        } else if (normalizedHeader.includes("debit_account") && field.value === "payment_account") {
          score = 95;
        } else if (normalizedHeader.includes("payment_account") && field.value === "payment_account") {
          score = 95;
        } else if (normalizedHeader.includes("payment") && normalizedHeader.includes("method") && field.value === "payment_method") {
          score = 95;
        } else if (normalizedHeader === "method" && field.value === "payment_method") {
          score = 85;
        } else if (normalizedHeader.includes("reference") && field.value === "payment_reference") {
          score = 85;
  ...
  const handleUploadFile = useCallback(() => {
    ...
    setIsUploading(true);
    router.post(route("receipts.import.upload"), formData, {
      onSuccess: () => {
        setCurrentStep(1);
      },
      onError: (errors) => {
  ...
  const requiredMappings = ["receipt_date", "product_name", "quantity", "unit_cost", "payment_account", "payment_method"];
  ...
  return (
    <AuthenticatedLayout>
      <Head title="Import Cash Invoices" />
      <SidebarInset>
            ...
            {currentStep === 0 && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Select an Excel or CSV file to import cash invoices</CardDescription>
                </CardHeader>
                <CardContent className="space-y-5">
                  ...
                  <Alert className="border-blue-500/30 bg-blue-500/5">
                    ...
                    <AlertDescription>
                      <h4 className="font-medium mb-2 text-blue-700">Multi-item Cash Invoice Import</h4>
                      <ul className="text-sm space-y-1 text-blue-600">
                        <li>- Duplicate <strong>Invoice Number</strong> values are supported and grouped into one cash invoice with multiple items</li>
                        <li>- Shared invoice fields such as date, customer, payment account, and payment method are taken from the first row in each invoice group</li>
                        <li>- Keep shared values consistent across grouped rows for predictable imports</li>
                      </ul>
                    </AlertDescription>
                  </Alert>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      <ul className="text-sm space-y-1">
                        ...
                        <li>- Required fields: Product Name, Quantity, Unit Cost, Invoice Date, Payment Account, Payment Method</li>
                        <li>- Optional <strong>Tax</strong> column: enter tax name exactly as configured in Tax Database</li>
                        <li>- Products are matched by name and stock-managed items must have enough stock available</li>
                        <li>- Customers, projects, payment accounts, and payment methods are matched by name</li>
                      </ul>
                    </AlertDescription>
                  </Alert>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Map your file columns to cash invoice fields</CardDescription>
                  </div>
                  <Button
                    type="button"
                    variant="outline"
            ...
            {currentStep === 2 && previewData && (
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <Card className="text-center p-4">
                    <p className="text-3xl font-bold text-blue-600">{previewData.unique_invoices || "-"}</p>
                    <p className="text-sm text-muted-foreground mt-1">Unique Invoices</p>
                  </Card>
                  <Card className="text-center p-4">
                ...
                {previewData.unique_invoices && previewData.unique_invoices < previewData.total_rows && (
                  <Alert className="border-blue-500/30 bg-blue-500/5">
                    ...
                    <AlertDescription className="text-blue-600">
                      <span className="font-medium">{previewData.total_rows} rows</span> will be grouped into <span className="font-medium">{previewData.unique_invoices} cash invoices</span> based on their invoice numbers.
                    </AlertDescription>
                  </Alert>
                )}
                          ...
                          {previewData.preview_records.map((record, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell className="text-muted-foreground">
                                {record.data.receipt_number || <span className="italic">-</span>}
                              </TableCell>
                              <TableCell className="text-muted-foreground">
