...
export default function Create({
  ...
  const [accountLines, setAccountLines] = useState([createEmptyLine()]);
  ...
  const resetForm = () => {
    reset();
    setExchangeRate(1);
    setAccountLines([createEmptyLine()]);
    setAttachments([]);
  };
  ...
  const submit = (event) => {
    ...
    post(route("hospital_purchases.store"), formData, {
      preserveScroll: true,
      onSuccess: () => {
        toast.success("Hospital purchase created successfully");
        resetForm();
      },
    });
  };
  ...
  return (
    <AuthenticatedLayout>
      ...
      <SidebarInset>
        <PageHeader page="Hospital Purchases" subpage="Create New" url="hospital_purchases.index" />
