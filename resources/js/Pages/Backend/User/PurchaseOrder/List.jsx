    ...
    <form onSubmit={onSubmit} className="p-6">
      ...
      <div className="ti-modal-body grid grid-cols-12">
        <div className="col-span-12">
          ...
          <input type="file" className="w-full dropify" name="purchase_orders_file" required />
        </div>
...
export default function List({ orders = [], meta = {}, filters = {} }) {
  ...
  const [showConvertToCashPurchaseModal, setShowConvertToCashPurchaseModal] = useState(false);
  ...
  const toggleSelectAll = (checked) => {
    ...
    setSelectedPurchaseOrders(checked ? orders.map(order => order.id) : []);
  };
  ...
  const handleDelete = (e) => {
    ...
    router.delete(route("purchase_orders.destroy", purchaseOrderToDelete), {
      onSuccess: () => {
        ...
        setProcessing(false);
        setSelectedPurchaseOrders(prev => prev.filter(id => id !== purchaseOrderToDelete));
        toast({
      ...
      onError: () => {
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "There was an error deleting the purchase order.",
        });
      }
    });
  };
    ...
    router.post(route("purchase_orders.convert_to_bill", purchaseOrderToDelete), {
      onSuccess: () => {
        ...
        toast({
          ...
          description: "purchase order has been converted to bill successfully.",
        });
      },
      onError: () => {
        setProcessing(false);
        toast({
          variant: "destructive",
          title: "Error",
          description: "There was an error converting the purchase order to bill.",
        });
      }
    });
  };
    ...
    router.post(route("purchase_orders.convert_to_cash_purchase", purchaseOrderToDelete), {
      onSuccess: () => {
        ...
        toast({
          ...
          description: "purchase order has been converted to cash purchase successfully.",
        });
      },
      onError: () => {
        setProcessing(false);
        toast({
          variant: "destructive",
          title: "Error",
          description: "There was an error converting the purchase order to cash purchase.",
        });
      }
    });
  };
    ...
    router.post(route("purchase_orders.import"), formData, {
      onSuccess: () => {
        ...
        toast({
          ...
          description: "Purchase orders have been imported successfully.",
        });
      },
      onError: (errors) => {
        setProcessing(false);
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuItem onClick={() => window.location.href = route("purchase_orders.export")}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="cursor-pointer" onClick={() => handleSort("order_number")}>Order Number {renderSortIcon("order_number")}</TableHead>
                    <TableHead className="cursor-pointer" onClick={() => handleSort("vendor.name")}>Supplier {renderSortIcon("vendor.name")}</TableHead>
                    <TableHead className="cursor-pointer" onClick={() => handleSort("order_date")}>Order Date {renderSortIcon("order_date")}</TableHead>
                    ...
                    <TableHead className="cursor-pointer text-right" onClick={() => handleSort("status")}>Status {renderSortIcon("status")}</TableHead>
                  ...
                  {orders.length > 0 ? (
                    orders.map((order) => (
                      <TableRow key={order.id}>
                        <TableCell>
                          <Checkbox
                            checked={selectedPurchaseOrders.includes(order.id)}
                            onCheckedChange={() => toggleSelectPurchaseOrder(order.id)}
                          />
                        </TableCell>
                        <TableCell>{order.order_number}</TableCell>
                        <TableCell>{order.vendor ? order.vendor.name : "-"}</TableCell>
                        <TableCell>{order.order_date}</TableCell>
                        <TableCell className="text-right">
                          {formatCurrency({ amount: order.grand_total, currency: order.currency })}
                        </TableCell>
                        <TableCell className="text-right">
                            ...
                            actions={[
                              {
                                ...
                                href: route("purchase_orders.show", order.id),
                              },
                              {
                                label: "Convert to bill",
                                icon: <FileUp className="h-4 w-4" />,
                                onClick: () => handleConvertToBillConfirm(order.id),
                              },
                              {
                                label: "Convert to Cash Purchase",
                                icon: <FileUp className="h-4 w-4" />,
                                onClick: () => handleConvertToCashPurchaseConfirm(order.id),
                              },
                              {
                                label: "Edit",
                                icon: <Edit className="h-4 w-4" />,
                                href: route("purchase_orders.edit", order.id),
                              },
                              {
                                label: "Delete",
