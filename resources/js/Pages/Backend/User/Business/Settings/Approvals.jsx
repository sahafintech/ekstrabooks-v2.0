...
export default function Approvals({ business, id, users = [], activeTab }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        hospital_purchase_approval_required_count: getSettingValue(
            ["hospital_purchase_approval_required_count", "purchase_approval_required_count"],
            "1"
        ),
        hospital_purchase_approval_users: getArraySettingValue([
            "hospital_purchase_approval_users",
            "purchase_approval_users",
        ]),
                        ...
                        <div className="max-w-4xl mx-auto">
                            <form onSubmit={submitApprovalSettings}>
                                ...
                                <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
                                    <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                                        <span className="mr-2">🏥</span>
                                        Hospital Purchases Approval Workflow
                                    </h3>

                                    <div className="grid grid-cols-12 mb-4">
                                        <Label htmlFor="hospital_purchase_approval_required_count" className="col-span-12 md:col-span-3 flex items-center mb-2 md:mb-0">
                                            Number of Approvals Required
                                        </Label>
                                        <div className="col-span-12 md:col-span-9">
                                            <Input
                                                id="hospital_purchase_approval_required_count"
                                                type="number"
                                                min="0"
                                                max="10"
                                                value={data.hospital_purchase_approval_required_count}
                                                onChange={(e) => setData("hospital_purchase_approval_required_count", e.target.value)}
                                                className="w-full"
                                                disabled={processing}
                                            />
                                            <p className="text-xs text-gray-500 mt-1">
                                                Minimum number of approvals needed before a hospital purchase is approved
                                            </p>
                                            <InputError message={errors.hospital_purchase_approval_required_count} className="mt-1" />
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-12 mb-4">
                                        <Label htmlFor="hospital_purchase_approval_users" className="col-span-12 md:col-span-3 flex items-center mb-2 md:mb-0">
                                            Approver Users
                                        </Label>
                                        <div className="col-span-12 md:col-span-9">
                                            <SearchableMultiSelectCombobox
                                                options={userOptions}
                                                value={data.hospital_purchase_approval_users}
                                                onChange={(values) => setData("hospital_purchase_approval_users", values)}
                                                placeholder="Select approver users"
                                                emptyMessage="No users found"
                                            />
                                            <p className="text-xs text-gray-500 mt-1">
                                                Users who are permitted to approve hospital purchases
                                            </p>
                                            <InputError message={errors.hospital_purchase_approval_users} className="mt-1" />
                                        </div>
                                    </div>
                                </div>
