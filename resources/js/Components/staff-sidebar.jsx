...
export function StaffSidebar(props) {
    const page = usePage();
    const url = page.url;
    const { auth, activeBusiness, permissionList, userPackage } = page.props;
    ...
    // HR & Payroll
    if (userPackage.payroll_module === 1) {
        if (
            [
                "staffs.index",
    ...
    // Construction
    if (userPackage.construction_module === 1) {
        if (
            [
                "projects.index",
                "project_groups.index",
                "cost_codes.index",
                "project_subcontracts.index",
            ].some((p) => perms.has(p))
        ) {
            const items = [];
            if (perms.has("projects.index")) {
                items.push({
                    title: "Projects",
            ...
            if (perms.has("project_groups.index")) {
                items.push({
            ...
            if (perms.has("project_subcontracts.index")) {
                items.push({
                    title: "Subcontracts",
